    ...
    try:
        ...
        if location:
            ...
            found_commune = address.get('city') or \
                            ...
                            address.get('county') # Ajout de county pour certains cas

            if found_commune:
                # Normalise les noms pour une meilleure comparaison (minuscules, sans accents, etc.)
                # Pour une comparaison simple, nous allons juste mettre en minuscules.
                # Pour une robustesse accrue, une librairie de normalisation de chaînes pourrait être utilisée.
...
if uploaded_file is not None:
    ...
    try:
        ...
        if not all(col in df.columns for col in required_columns):
            ...
            if not false_matches_df.empty:
                ...
                st.download_button(
                    ...
                    data=false_matches_df.to_csv(index=False).encode('utf-8'),
        ...
        st.info("Veuillez vous assurer que votre fichier est bien un CSV valide et contient les colonnes 'latitude', 'longitude' et 'commune'.")
